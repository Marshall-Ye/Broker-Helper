# main_ui.py — Main launcher window with tabbed GUI for all features
# ---------------------------------------------------------------
# Tabs:
# • Excel Splitter: splits CI into customs-ready chunks
# • Reject Code Sorter: extracts line# by message ID from reject PDFs
# • PGA Reference: viewable static guide blocks
# • JSON Converter: prepares and submits data to Acelynk
# ---------------------------------------------------------------
...
def resource_path(relative_path: str) -> Path:
    """Returns absolute path to bundled resource (handles PyInstaller freeze)"""
...
class ExcelSplitterTab:
    def __init__(self, parent):
        ...
        self.rows_var = ctk.StringVar(value="495")  # default value
...
class MainApp(TkinterDnD.Tk):
    def __init__(self):
        ...
        JsonConverterTab(self.tabview.add("JSON Converter"), OUT_DIR)  # pass output folder       ✅ added module

        # bottom-left logo (optional)
        if BANNER.exists():
            img = Image.open(BANNER)
            small = ctk.CTkImage(light_image=img, dark_image=img,
                                 size=(img.width//4, img.height//4))
            ctk.CTkLabel(self, image=small, text="").grid(
                row=1, column=0, sticky="w", padx=(20, 0), pady=(0, 20)
            )
        else:
            ctk.CTkLabel(self, text="").grid(
                row=1, column=0, sticky="w", padx=(20, 0), pady=(0, 20)
            )
